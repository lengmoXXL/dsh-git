...
- **内容宽度**放在**滚动内容**自己身上（`.laneRows`：`width: max-content; min-width: 100%`），不是放在 lane 的轨道上：lane 本身是固定宽度的滚动容器，轨道没有剩余空间可长，`max-content` 轨道会停在半栏宽 ✗——于是正文溢出到行框之外，而行的染色带只有行框那么宽，**一横向滚动，色带就一点点被推出半栏、最后完全消失**，正文却还在。宽度写在滚动内容上，才保证"至少半栏宽、最长行时更宽"，滚动到任何位置色带都铺满半栏。
...
**每个 bundle 都带构建时间戳**（`tsdown.config.ts` 在编译时把 `__DSH_GIT_BUILD__` 替换掉，`src/client/build.ts` 暴露它）：把鼠标停在 diff 头部那个 `abc^ → abc` 上、或日志页的仓库名上，就能看到这一页正在跑哪个时间的构建。之所以需要它：**diff 的内容由 host 现读（总是最新），而布局来自浏览器加载的那个 bundle**——页面没跟上时就会出现"新内容用旧布局画"，看起来像布局 bug。

- **滚动状态**（横向滚到一半时行号是否遮住正文、色带是否跟上）：`qlmanage` 不执行 JS，负边距只能算近似（它移动的是你选中的那个盒子，而真滚动移动的是整个滚动内容）——**曾因此漏掉"色带随滚动变窄"这个 bug**。现在用真实浏览器量：本机 `~/Library/Caches/ms-playwright` 里有 chromium，`node /tmp/measure.mjs <page.html> <expr.js>` 通过 CDP 载入页面、滚到最右、读 `getBoundingClientRect()`（Node 自带 WebSocket，无需额外依赖）。修复前色带右边缘在滚动到底时距半栏右边缘 **2790px**，修复后 **0.3px**。
