...
- **自动折行** / **不折行**：折行时长行在各自半栏里绕行；不折行时保留整行，**每一栏各自横向滚动**（像编辑器那样）。两种模式里两栏都始终看得见。默认折行。
...
- 布局不变量在 markup 里看不见（行塌陷是"计算高度"、行号遮挡是"绘制顺序"、色带宽度是"轨道尺寸"），所以它们由 `client-bundle.test.ts` 直接对**构建产物里的 CSS** 断言，而不是对渲染出的 HTML 断言。断言只能证明声明在，证明不了算出来的结果，所以改这类地方时要**在真实浏览器里量** `getBoundingClientRect()`、并**把滚动到两端的状态都量一遍**——这个项目里被漏掉过的 bug（色带随横向滚动变窄直至消失）就是这样漏掉的。
