...
- **不折行**：长行不折，**每一栏各自横向滚动**（像编辑器那样），行号用 `position: sticky` 钉在各自半栏的左边、不跟着滚走；左右两栏不会因为某一侧的行长而互相挤。两栏各自是一列**固定高度的网格行**（`grid-auto-rows`）——这一点不是装饰：某一行在它自己那一侧没有内容时（插入/删除的空半边）什么都画不出来，若行高由内容撑，那一行就会塌成 0 高，两栏每遇到一个空行就错开一行，从第一个插入开始两边读起来就是两段不同的代码。
